...
var roi = d.us_underc;
var ctr = ee.Geometry.Point([-89.5278, 46.2432]);
...
var emit_collection = ee.ImageCollection('NASA/EMIT/L2A/RFL').select(ee.List.sequence(0, 284)).select(b.bands_emit_orig,b.bands_emit_mod).filterBounds(ctr);
...
var controlPanel = ui.Panel({widgets: [
    ui.Label({value: 'ISS EMIT image selection', style: {fontSize: '14px', fontWeight: 'bold', color: '#000000'}}),
    ui.Label({value: 'Note: Not all images listed cover the full site.', style: {fontSize: '11px', color: 'grey', fontStyle: 'italic'}})],
...
function exportSpectralData() {
    ...
    var downloadUrl = sampledData.getDownloadURL({format: 'CSV', filename: 'emit_spectral_signature_' + Date.now()});
...
function updateMapWithImage(imageId) {
    ...
    visStats.evaluate(function(result) {
        ...
        Map.addLayer(d.us_underc_ln1,{palette: "red"}, "UNDERC NEON site boundaries", 1);

        //Map.centerObject(imageGeometry, 10); // Center map on the image
        Map.setCenter(-89.522, 46.2373, 13); // Center map over the UNDERC NEON site
...
imageSelect.setValue(imageList[2]);
...
function updateSpectralChart() {
    ...
    .setOptions({title: chartTitle, titleTextStyle: {fontSize: 14, bold: true},
        hAxis: {title: 'Wavelength (nm)', titleTextStyle: {fontSize: 11}}, vAxis: {title: 'Reflectance (scaled 0-1)', titleTextStyle: {fontSize: 11}},
        series: (function() {
            var series = {};
