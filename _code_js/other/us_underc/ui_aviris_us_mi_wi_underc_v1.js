/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 
// AVIRIS-NG hyperspectral data viewer
...
var emit_collection = b.aviris_underc_renamed.filterBounds(ctr);
...
var viz_emit = {bands: ['ρ1605', 'ρ0864', 'ρ0654' ], min: [-0.0730, -0.0834, -0.0684], max:[0.4050, 0.5376, 0.2140]};
...
var titlePanel = ui.Panel({
    widgets: [
        ui.Label({value: "Hyperspectral Data Explorer: AVIRIS-NG", style: {fontSize: '20px', fontWeight: 'bold', color: 'chocolate', margin: '8px 0px'}}),
...
var controlPanel = ui.Panel({widgets: [
    ui.Label({value: 'AVIRIS-NG image selection', style: {fontSize: '14px', fontWeight: 'bold', color: '#000000'}})],
...
function exportSpectralData() {
    ...
    var downloadUrl = sampledData.getDownloadURL({format: 'CSV', filename: 'aviris_ng_spectral_signature_' + Date.now()});
...
function updateMapWithImage(imageId) {
    ...
    visStats.evaluate(function(result) {
        var visParams = {bands: ['ρ1605', 'ρ0864', 'ρ0654' ], min: [-0.0730, -0.0834, -0.0684], max:[0.4050, 0.5376, 0.2140]};
        
        // Create layer name with mask info
        var layerName = 'AVIRIS-NG scene';
...
imageSelect.setValue(imageList[1]); 
...
var bandOptions = [];
var bandOptions = b.aviris_ng_bands_mod;

// RGB Band selectors
var redBandSelect = ui.Select({items: bandOptions, value: 'ρ1605', placeholder: 'Select Red band...', style: {margin: '2px 0px', width: '80px'}});
var greenBandSelect = ui.Select({items: bandOptions, value: 'ρ0864', placeholder: 'Select Green band...', style: {margin: '2px 0px', width: '80px'}});
var blueBandSelect = ui.Select({items: bandOptions, value: 'ρ0654', placeholder: 'Select Blue band...', style: {margin: '2px 0px', width: '80px'}});
...
function updateSpectralChart() {
    ...
    var chartTitle = 'Surface reflectance (AVIRIS-NG bands 1-426)';
    
    var chart = ui.Chart.image.regions({
        image: currentSpectralImage,  // Always uses original spectral bands only
        regions: pointsCollection, reducer: ee.Reducer.mean(), scale: 3, seriesProperty: 'ρointId', xLabels: b.wl_aviris_ng})
