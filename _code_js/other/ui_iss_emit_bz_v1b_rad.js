...
var emit_collection = ee.ImageCollection('NASA/EMIT/L1B/RAD').select(ee.List.sequence(0, 284)).select(b.bands_emit_rad_orig,b.bands_emit_mod).filterBounds(c.roi_bz);
...
var viz_emit = {bands: ['ρ1603', 'ρ0865', 'ρ0656' ], min: [0.52, 5.69, -6.41], max: [3.61, 19.76, 23.57]};
...
var spectralPanel = ui.Panel({widgets: [
        ...
        ui.Label({value: 'Color order: red, green, blue, yellow, magenta, cyan, orange, purple, black', style: {fontSize: '9px', color: 'grey', fontStyle: 'italic'}}),
        ],
...
function updateMapWithImage(imageId) {
    ...
    visStats.evaluate(function(result) {
        var visParams = {bands: ['ρ1603', 'ρ0865', 'ρ0656' ], min: [0.52, 5.69, -6.41], max: [3.61, 19.76, 23.57]};
...
function updateSpectralChart() {
    ...
    var chartTitle = 'Radiance (ISS EMIT bands 1-285)';
    ...
    .setOptions({title: chartTitle, titleTextStyle: {fontSize: 14, bold: true},
        hAxis: {title: 'Wavelength (nm)', titleTextStyle: {fontSize: 11}}, vAxis: {title: 'Radiance', titleTextStyle: {fontSize: 11}},
        series: (function() {
            var series = {};
