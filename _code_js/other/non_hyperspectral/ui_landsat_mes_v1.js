/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Multispectral Data Explorer: Landsat
...
var wl_landsat = [480, 560, 655, 865, 1610, 2200];
...
var landsat_collection = b.img.map(function(img)
{return img.select(['B1','B2','B3','B4','B5','B7'],['ρ0480', 'ρ0560', 'ρ0655', 'ρ0865', 'ρ1610', 'ρ2200'])});
//print('Landsat collection size:', landsat_collection.size());


// Get collection info for dropdown
var imageList = landsat_collection.aggregate_array('system:index').getInfo();
var dateList = landsat_collection.aggregate_array('system:time_start').map(function(timestamp) {return ee.Date(timestamp).format('YYYY-MM-dd')}).getInfo();

// Create a styled title panel
var titlePanel = ui.Panel({
    widgets: [
        ui.Label({value: "Multispectral Data Explorer: Landsat 5-9", style: {fontSize: '20px', fontWeight: 'bold', textAlign:'left', color: 'green', margin: '8px 0px'}}),
        ui.Label("viewer based on Dr. Sam Roy's TANAGER Viewer", {stretch:'horizontal',textAlign:'left',fontSize: '12px', fontStyle: 'italic', color: 'lightskyblue'},
["https://sat-io.earthengine.app/view/tanager"]),
        ui.Label("credits: Landsat imagery © NASA, USGS; processed with Kennedy et al. (2018)'s LandTrendr algorithm", {stretch:'horizontal',textAlign:'left',fontSize: '10px', fontStyle: 'italic', color: 'grey'})],
    style: {position: 'top-left', padding: '12px', backgroundColor: 'white', border: '2px solid #000000'}});

// Create control panel
var controlPanel = ui.Panel({widgets: [ui.Label({value: 'Landsat mosaic selection', style: {fontSize: '14px', fontWeight: 'bold', color: '#000000'}})],
    style: {position: 'bottom-left', padding: '10px', backgroundColor: '#FFFFFF', border: '1px solid #000000',  width: '350px'}});
...
function exportSpectralData() {
    ...
    var downloadUrl = sampledData.getDownloadURL({format: 'CSV', filename: 'landsat_spectral_signature_' + Date.now()});
...
function calculateMinMax(image, geometry) {
    var stats = image.select(['ρ1610', 'ρ0865', 'ρ0655']).reduceRegion({reducer: ee.Reducer.minMax(), geometry: geometry, scale: 100, maxPixels: 1e9});
    return stats}
...
function updateMapWithImage(imageId) {
    ...
    var selectedImage = landsat_collection.filterMetadata('system:index', 'equals', imageId).first();
    ...
    visStats.evaluate(function(result) {
        var visParams = {bands: ['ρ1610', 'ρ0865', 'ρ0655'], min: [730, 1800, -740], max:[2200, 3800, 2000]};
        
        // Create layer name with mask info
        var layerName = 'Landsat dry season mosaic';
        
        Map.setControlVisibility({layerList: true, zoomControl: false, mapTypeControl: true, fullscreenControl: true, drawingToolsControl: false});
        Map.addLayer(displayImage, visParams, layerName, true); // Add the hyperspectral layer
        Map.addLayer(a.bnds_intl_ln1,{palette: "white"},"Int'l boundaries (white)", 1);
        //Map.addLayer(a.bnds_admin_l1_ln1_us, {palette: "silver"},"State boundaries - USA (silver)", 1);
        Map.setCenter(-87.7459, 16.9767, 8); // Center map over Central America
        //Map.setCenter(-86.105, 14.099, 8);
...
imageSelect.setValue(imageList[41]);
...
var bandOptions = [];
var bandOptions = ['ρ0480', 'ρ0560', 'ρ0655', 'ρ0865', 'ρ1610', 'ρ2200'];

// RGB Band selectors
var redBandSelect = ui.Select({items: bandOptions, value: 'ρ1610', placeholder: 'Select Red band...', style: {margin: '2px 0px', width: '80px'}});
...
var blueBandSelect = ui.Select({items: bandOptions, value: 'ρ0655', placeholder: 'Select Blue band...', style: {margin: '2px 0px', width: '80px'}});
...
function updateVisualizationWithSelectedBands() {
    ...
    var selectedImage = landsat_collection.filterMetadata('system:index', 'equals', spectralPanel.selectedImageId).first();
    var displayImage = selectedImage;
...
function updateSpectralChart() {
    ...
    var chartTitle = 'Surface reflectance (Landsat TM / ETM+ / OLI / OLI-2 bands)';
    
    var chart = ui.Chart.image.regions({
        image: currentSpectralImage,  // Always uses original spectral bands only
        regions: pointsCollection, reducer: ee.Reducer.mean(), scale: 100, seriesProperty: 'ρointId', xLabels: wl_landsat})
...
var leftPanel = ui.Panel({widgets: [spectralPanel], layout: ui.Panel.Layout.flow('vertical'), style: {position: 'top-right'}});
