...
// Credits: This code is mostly from Dr. Samapriya Roy from his TANAGER Hyperspectral Data Viewer (see: https://sat-io.earthengine.app/view/tanager).
...
//
var emit_collection = ee.ImageCollection('NASA/EMIT/L2A/RFL').select(ee.List.sequence(0, 284)).select(b.bands_emit_orig,b.bands_emit_mod).filterBounds(c.roi_sv_san_salvador);
...
//var emit_collection = pace_oci_mt_global;
...
var titlePanel = ui.Panel({
    widgets: [
        ui.Label({value: "ISS EMIT - hyperspectral data viewer", style: {fontSize: '20px', fontWeight: 'bold', color: 'chocolate', margin: '8px 0px'}}),
        ui.Label("veiwer based on Dr. Sam Roy's TANAGER Viewer", {stretch:'horizontal',textAlign:'center',fontSize: '12px', fontStyle: 'italic', color: 'lightskyblue'},
...
var controlPanel = ui.Panel({widgets: [ui.Label({value: 'EMIT image selection', style: {fontSize: '14px', fontWeight: 'bold', color: '#000000'}})],
...
function updateMapWithImage(imageId) {
    ...
    visStats.evaluate(function(result) {
        ...
        var layerName = 'ISS EMIT mosaic';
        
        Map.addLayer(displayImage, visParams, layerName, true); // Add the hyperspectral layer
        Map.addLayer(c.bnds_intl_cam_ln2,{palette: "white"},"Int'l boundaries (white)", 1);
        //Map.centerObject(imageGeometry, 11); // Center map on the image
        Map.setCenter(-89.1994, 13.7026, 12); // Center map over San Salvador
...
imageSelect.setValue(imageList[6]);
