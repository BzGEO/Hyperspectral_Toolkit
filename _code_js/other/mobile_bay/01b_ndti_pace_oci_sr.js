...
function ndti_sr(img) {return img.normalizedDifference(['b60','b43']).rename('NDTI').set(t,img.get(t))}
...
//var roi = loc1_box1km;
var roi = ee.FeatureCollection([loc1_box1km,loc2_box1km,loc3_box1km]);
...
print(ui.Chart.image.seriesByRegion({imageCollection: a.pace_oci_mt_conus.map(ndti), regions: roi, reducer: ee.Reducer.mean(),
scale: 500, band: 'NDTI', xProperty: t}).setOptions({title: 'PACE OCI - NDTI', lineWidth: 1}));


print(ui.Chart.image.series(a.pace_oci_mt_conus.map(ndti), roi, ee.Reducer.mean(), 4000)
    .setOptions({title: 'PACE OCI - NDTI', lineWidth: 1, pointSize: 3,
    vAxis: {title: 'NDTI'}, series:{0:{color:'limegreen'}}}));

// pace_oci_rrs_mt_conus
