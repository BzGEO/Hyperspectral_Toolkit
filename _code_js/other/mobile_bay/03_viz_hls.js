...
function mosaicDates(images) {
  ...
  return i.reproject({crs: ee.Projection('EPSG:4326').atScale(100)}).set({date:i.date().format('YYYY-MM-dd')})}); //set date for all images
...
var hls = ee.ImageCollection("NASA/HLS/HLSL30/v002")
              ...
              .filterBounds(roi)
              .map(function(img){return img.multiply(10000).toInt16()
...
var collection = hls.filterBounds(roi)
                    ...
                    .sort(t, false)
                    .map(function(image){var dateStr = ee.Date(image.get(t)).format('YYYY-MM-dd');
