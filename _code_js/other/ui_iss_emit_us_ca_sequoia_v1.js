/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 
// ISS EMIT hyperspectral data viewer
...
var d = require('users/bzgeo/hyperspectral_toolkit:00_pkg/sample_sites.js');

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

var roi = d.roi_ca_snp;
...
// Load ISS EMIT collection
var emit_collection = ee.ImageCollection('NASA/EMIT/L2A/RFL').select(ee.List.sequence(0, 284)).select(b.bands_emit_orig,b.bands_emit_mod).filterBounds(roi);
//print(emit_collection);
//print('ISS EMIT collection size:', emit_collection.size());

//
//var viz_emit = {bands: ['reflectance_164','reflectance_65','reflectance_37'], min: [-0.0730, -0.0834, -0.0684], max:[0.4050, 0.5376, 0.2140]};
var viz_emit = {bands: ['ρ1603', 'ρ0865', 'ρ0656' ], min: [-0.0730, -0.0834, -0.0684], max:[0.4050, 0.5376, 0.2140]};

// Get collection info for dropdown
var imageList = emit_collection.aggregate_array('system:index').getInfo();
var dateList = emit_collection.aggregate_array('system:time_start').map(function(timestamp) {return ee.Date(timestamp).format('YYYY-MM-dd')}).getInfo();

// Create a styled title panel
var titlePanel = ui.Panel({
    widgets: [
        ui.Label({value: "Hyperspectral Data Explorer: EMIT", style: {fontSize: '20px', fontWeight: 'bold', color: 'chocolate', margin: '8px 0px'}}),
...
var controlPanel = ui.Panel({widgets: [ui.Label({value: 'ISS EMIT image selection', style: {fontSize: '14px', fontWeight: 'bold', color: '#000000'}})],
...
function exportSpectralData() {
    ...
    var sampledData = currentImage.sampleRegions({collection: pointsFC, scale: 100, geometries: true});
    ...
    downloadPanel.add(ui.Label({value: 'Contains: Point IDs, coordinates, and EMIT bands 1-285', style: {fontSize: '10px', color: '#666666', margin: '5px 0px 0px 0px'}}))}
...
function calculateMinMax(image, geometry) {
    var stats = image.select(['ρ1618', 'ρ0835', 'ρ0662']).reduceRegion({reducer: ee.Reducer.minMax(), geometry: geometry, scale: 100, maxPixels: 1e9});
    return stats}
...
function updateMapWithImage(imageId) {
    ...
    var selectedImage = emit_collection.filterMetadata('system:index', 'equals', imageId).first();
    ...
    visStats.evaluate(function(result) {
        var visParams = {bands: ['ρ1603', 'ρ0865', 'ρ0656' ], min: [-0.0730, -0.0834, -0.0684], max:[0.4050, 0.5376, 0.2140]};
        
        // Create layer name with mask info
        var layerName = 'ISS EMIT scene';
        
        Map.addLayer(displayImage, visParams, layerName, true); // Add the hyperspectral layer
        
        Map.addLayer(a.fire(a.mcd64a1_2021, roi), {color: "red"}, "MODIS burn scars (2021)", 1);
        Map.addLayer(a.fire(a.mcd64a1_2022, roi), {color: "red"}, "MODIS burn scars (2022)", 0);
        Map.addLayer(a.fire(a.mcd64a1_2023, roi), {color: "red"}, "MODIS burn scars (2023)", 0);
        Map.addLayer(a.fire(a.mcd64a1_2024, roi), {color: "red"}, "MODIS burn scars (2024)", 0);
        Map.addLayer(a.fire(a.mcd64a1_2025, roi), {color: "red"}, "MODIS burn scars (2025)", 0);
        
        Map.addLayer(c.bnds_admin_l1_ln1_us, {palette: "silver"},"State boundaries - US (silver)", 1);
        Map.addLayer(c.us_pad_ln1,{}, "US - PAD", 0);

        Map.centerObject(imageGeometry, 10); // Center map on the image
...
// Create band options for dropdowns (bands 1-285)
var bandOptions = [];
var bandOptions = b.bands_emit_mod;

// RGB Band selectors
var redBandSelect = ui.Select({items: bandOptions, value: 'ρ1603', placeholder: 'Select Red band...', style: {margin: '2px 0px', width: '80px'}});
var greenBandSelect = ui.Select({items: bandOptions, value: 'ρ0865', placeholder: 'Select Green band...', style: {margin: '2px 0px', width: '80px'}});
var blueBandSelect = ui.Select({items: bandOptions, value: 'ρ0656', placeholder: 'Select Blue band...', style: {margin: '2px 0px', width: '80px'}});
...
function calculateMinMaxForBands(image, bands, geometry) {
    var stats = image.select(bands).reduceRegion({reducer: ee.Reducer.minMax(), geometry: geometry, scale: 100, maxPixels: 1e9});
    return stats}
...
function updateVisualizationWithSelectedBands() {
    ...
    var selectedImage = emit_collection.filterMetadata('system:index', 'equals', spectralPanel.selectedImageId).first();
    var displayImage = selectedImage;
...
function updateSpectralChart() {
    ...
    var chartTitle = 'Surface reflectance (ISS EMIT bands 1-285)';
    
    var chart = ui.Chart.image.regions({
        image: currentSpectralImage,  // Always uses original spectral bands only
        regions: pointsCollection, reducer: ee.Reducer.mean(), scale: 100, seriesProperty: 'ρointId', xLabels: b.wl_emit})
    .setOptions({title: chartTitle, titleTextStyle: {fontSize: 14, bold: true},
        hAxis: {title: 'Wavelength (nm)', titleTextStyle: {fontSize: 11}}, vAxis: {title: 'Surface reflectance (scaled 0-1)', titleTextStyle: {fontSize: 11}},
        series: (function() {
            var series = {};
